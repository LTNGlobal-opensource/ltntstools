...
- v1.39.14
  ...
  rpm: Ensure all libraries in this packages are excluded from RPMS 'provides' solution.
...
- v1.38.5
  ...
  tstools_nic_monitor: Added curl as a dep. Fully enable the --http-json-reporting feature.
  tstools_nic_monitor: Limited support for SRT monitoring in PCAP mode only.
  tstools_clock_inspector: Set the current time and report it, if we're not getting the time from the PCR
  tstools_clock_inspector: Output stream name when reporting a CC error.
  ...
  tstools_clock_inspector: Remove a spurious avio_read() call that goes to console.
...
- v1.36.4
  ...
  libltntstools: pes extraction updates fixing weird ac3 decoding issues
...
- v1.34.2
  ...
  core: Experimental - support for Statics and packet out of order detection.
...
- v1.33.1
  ...
  tstools_clock_inspector: When printing a linear trend, show date and time for easier scripting.
  tstools_clock_inspector: Don't initialize a DTS trend unless a DTS is detected
  tstools_clock_inspector: Hide all of the trend supporting data printing, behind multiple -L invocations.
  tstools_clock_inspector: Use multiple -L optoins to save content to disk (autogenerated filename)
  tstools_clock_inspector: Compute and report on how many SCR ticks and walltime uS it takes to receive an entire PES.
...
- v1.31.1
  ...
  tstools_pes_inspector: Order PES callbacks for PIC_TIMING in proper temporal order.
  ...
  tstools_nic_monitor: Added support for -i udp://address:port multicast inputs
